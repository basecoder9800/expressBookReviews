...
public_users.post("/register", (req,res) => {
  ...
  return res.status(404).json({message: "Unable to register this user."});
});
...
public_users.get('/author/:author',function (req, res) {
  ...
  let ans = []
    ...
    if(ans.length == 0){
        return res.status(300).json({message: "Unable to find given author."});
    }
    res.send(ans);
});
...
public_users.get('/title/:title',function (req, res) {
  ...
  if(ans.length == 0){
      return res.status(300).json({message: "Unable to find given title."});
  }
  res.send(ans);
});
